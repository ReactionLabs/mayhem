...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    safeLogError('AI Generation error', error);
...
async function generateCoin(prompt: string): Promise<any> {
  try {
    ...
    console.error('OpenAI coin generation error:', error);
    throw new Error(`Failed to generate coin: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
