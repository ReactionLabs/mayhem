...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Close
    ref={ref}
    className={cn(
      'flex items-center justify-center rounded-lg p-2 text-muted-foreground hover:bg-accent hover:text-accent-foreground focus:outline-primary transition-colors',
      className
    )}
    {...props}
  >
...
>(({ className, children, animate = true, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-1/2 top-1/2 z-50 grid w-full max-w-lg -translate-x-1/2 -translate-y-1/2 rounded-xl border border-border bg-background shadow-lg duration-200 sm:rounded-lg',
        animate &&
...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div className={cn('text-center sm:text-left space-y-1.5 p-6', className)} {...props} />
);
...
const DialogSubmitButton: React.FC<React.ComponentProps<'button'>> = ({ className, ...props }) => {
  return (
    <button
      className={cn(
        'w-full rounded-lg bg-primary py-2.5 text-base font-semibold text-primary-foreground',
        'enabled:hover:bg-primary/90 transition-colors',
        'disabled:opacity-50',
        className
      )}
      {...props}
